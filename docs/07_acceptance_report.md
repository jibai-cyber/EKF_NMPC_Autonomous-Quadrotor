...
最终验收运行共记录 7750 个样本，仿真时长 77.50 s。八字阶段的三维位置跟踪 RMSE 为
0.0510 m，入口前 5 s 的三维 RMSE 为 0.0490 m、最大误差为 0.0708 m。八字阶段内
旋翼推力、机体力矩、倾角和机体系角速度均无约束越界。NMPC 日志样本成功率为
99.9675%，中位求解时间为 12.88 ms，P95 为 18.01 ms。
...
$F_k$ 及 IMU 输入 Jacobian $G_k$ 通过中心差分计算，协方差预测为

$$
P_{k+1}^-=F_kP_k^+F_k^T+Q_k.
$$

$Q_k$ 包含 $G_kR_{imu}G_k^T$，以及按定加速度模型精确离散的位置—速度相关过程噪声。
North/East/Down 的连续白加速度模型误差密度采用 $[0.50,0.08,0.08]$
m/s²/$\sqrt{\mathrm{Hz}}$；North 分量根据 X 轴一致性结果进行工程整定。
...
| yaw 对准与状态确认 | 5-10.52 s | 对准 $53.13^\circ$，并等待状态稳定 0.5 s |
| 三维八字 | 10.52-72.04 s | 解析八字空间路径和 3 s 相位速度渐入 |
...
- 图表按日志中的有效任务时间识别八字阶段，最终释放时刻为 10.52 s；
- 原始日志以约 100 Hz 保存，最终运行记录到 77.50 s。
...
| 指标 | X | Y | Z | 三维范数 |
|---|---:|---:|---:|---:|
| 八字全阶段 RMSE | 0.0354 m | 0.0350 m | 0.0109 m | 0.0510 m |
| 八字全阶段最大绝对值/范数 | 0.1005 m | 0.0999 m | 0.0328 m | 0.1282 m |
| 入口 5 s RMSE | 0.0357 m | 0.0323 m | 0.0090 m | 0.0490 m |
| 入口 5 s 最大三维误差 | - | - | - | 0.0708 m |
| 入口后稳态 RMSE | 0.0353 m | 0.0353 m | 0.0110 m | 0.0511 m |

修改前入口速度阶跃产生 0.5849 m 最大三维误差和 0.2369 m 的入口 5 s RMSE。平滑相位
渐入后分别降为 0.0708 m 和 0.0490 m；入口 RMSE 与后续稳态 0.0511 m 基本一致，说明
原有可见瞬态已经消除。

八字阶段航向 RMSE 为 $3.750^\circ$。高度方向 RMSE 为 0.0109 m，明显小于水平误差，
表明垂向跟踪和重力补偿稳定。
...
三个方向的估计曲线均未发散。结构化过程噪声离散和 North 模型误差整定将 X 轴覆盖率从
82.42% 提高到 92.09%，已接近高斯假设下 $\pm2\sigma$ 的理论 95%；Y、Z 两轴保持在
97% 以上。X 轴仍略偏乐观，因此报告保留实际覆盖率，不宣称完全一致。
...
|---|---:|---:|---:|
| 最小旋翼推力 | 2.3716 N | $\ge0.2$ N | 0 |
| 最大旋翼推力 | 3.5667 N | $\le5.5$ N | 0 |
| 最大相邻推力变化 | 0.5443 N/step | $\le2.0$ N/step | 0 |
| $|\tau_x|$ | 0.0500 N m | $\le0.05$ N m | 0 |
| $|\tau_y|$ | 0.0500 N m | $\le0.05$ N m | 0 |
| $|\tau_z|$ | 0.0150 N m | $\le0.015$ N m | 0 |

旋翼推力距离上限仍有 1.9333 N 裕度，距离下限有 2.1716 N 裕度。实际最大相邻命令变化
为 0.5443 N，低于预测约束的 2.0 N/step。三轴力矩多次接近
...
|---|---:|---:|---:|---:|
| 倾角 $\max(|\phi|,|\theta|)$ | 6.44° | 35° | 28.56° | 0 |
| $|p|$ | 40.74°/s | 180°/s | 139.26°/s | 0 |
| $|q|$ | 44.18°/s | 180°/s | 135.82°/s | 0 |
| $|r|$ | 56.35°/s | 90°/s | 33.65°/s | 0 |
...
绝大多数求解在 20 ms 内完成，满足 50 ms 控制周期；但仍有 3 个日志样本超过控制周期，
...
- Docker 构建成功，20 项自动化测试全部通过。

### 10.2 当前限制

1. X 轴 $\pm2\sigma$ 覆盖率已提高到 92.09%，但仍略低于理论 95%。
2. 三轴力矩经常接近限制，虽然未越界，但控制平滑性仍有提升空间。
3. NMPC 存在少量超过 50 ms 控制周期的样本，尚不能称为硬实时保证。
4. 模型未包含气动阻力、风扰动和电机一阶动态。
5. 13 维 EKF 不估计 IMU bias，因此无法补偿真实硬件的慢变零偏，也不满足 rubric
   最高档对 bias estimation 的完整要求。

### 10.3 改进落实情况

1. 已使用离散 IMU 输入 Jacobian和定加速度精确离散修正 $Q_k$，X 轴覆盖率提高到 92.09%；
