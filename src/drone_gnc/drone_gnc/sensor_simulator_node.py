"""Add the specified noise/biases and expose NED/FRD sensor topics."""
...
class SensorSimulatorNode(Node):
    def __init__(self) -> None:
        ...
        self.accel_bias_rw = self.declare_parameter("accel_bias_rw_std", 0.002).value
        self.gyro_bias_rw = self.declare_parameter("gyro_bias_rw_std", 0.0002).value
    ...
    def imu_callback(self, message: Imu) -> None:
        ...
        dt_s = (
            ...
            else np.clip(now_s - self.last_imu_time, 1e-4, 0.1)
        )
        self.last_imu_time = now_s
        self.accel_bias += self.accel_bias_rw * math.sqrt(dt_s) * self.rng.standard_normal(3)
        self.gyro_bias += self.gyro_bias_rw * math.sqrt(dt_s) * self.rng.standard_normal(3)
        ...
        accel_frd += self.accel_bias + self.accel_std * self.rng.standard_normal(3)
        gyro_frd += self.gyro_bias + self.gyro_std * self.rng.standard_normal(3)
    ...
    def odometry_callback(self, message: Odometry) -> None:
        ...
        output.body_rates_frd_radps = vector_flu_to_frd(angular_rate_flu).tolist()
